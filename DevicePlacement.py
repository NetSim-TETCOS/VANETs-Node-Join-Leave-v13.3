...
NodeStatus=file_for_writing[:-C]

roadfile = roadfile +"RoadsPlacement.txt"	#Roads placement file in same path. This will have road's coordinates multiplied by a factor
NodeStatus = NodeStatus + "mobility.txt" # writes the status of the node whether it is in the network or left the network
...
arrived_no=0						#This variable adds up on finding a vehicle that arrived at each point of sumo time
#time.sleep(5.0)
...
fstatus.write("#The Format: Vehicle Name, Time of arrival/departure, Node either joined (or) left the simulation\n")
fstatus.write("# Example : Vehicle_1,0.0,Join\n")
fstatus.write("# Example : Vehicle_1,50.0,Leave\n")
while step < float(B2[1]):			#As long as it is less than Simulation time
   #time.sleep(1.0)
	...
	curr_arrival_list=traci.simulation.getArrivedIDList()		#List of Vehicles arrived in this simulation step
	curr_arrival_no=traci.simulation.getArrivedNumber()		#No. of vehicles arrived in rthis simulation step
	arrived_no=arrived_no+curr_arrival_no					#Add it in the Total vehicles arrived
	...
	for i in curr_departed_list:
		...
		fstatus.write(i)
		fstatus.write(",")
		fstatus.write(str(step))	
		fstatus.write(",")	
		fstatus.write("Join\n")		
		l=l+1
	for i in curr_arrival_list:
		fstatus.write(i)
		fstatus.write(",")
		fstatus.write(str(step))	
		fstatus.write(",")	
